...
m = re.search(r'<table class="table table-striped.*">(.*?)</table>', d, flags=re.MULTILINE | re.DOTALL)
assert m, "Can't find table"

d = m[1]

header = sc.find("<tr>\s*(<th><strong>Date</strong></th>\s*<th><strong>Personnes hospitalisées</strong></th>\s*<th><strong>Total Décès</strong></th>\s*<th><strong>Total cas avérés</strong></th>)\s*</tr>", d)
