...
echo "$d" | egrep "Stand" | tr '\240' ' ' | sed -E -e 's/^.*Stand [[:alpha:]]+, (.+ Uhr), .*$/\1/'

echo -n "Confirmed cases: "
echo "$d" | egrep "Es sind jetzt insgesamt" | tr '\240' ' ' | sed -E -e 's/^.*Es sind jetzt insgesamt ([0-9]+) bestätigte Fälle.*$/\1/'
